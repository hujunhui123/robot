...
public interface CLibrary extends StdCallLibrary {
    ...
    //初始化函数
    void init(ResultStruct pResultStruct,String RemoteAddr,String UserName,String Pass,String RobotId);
    //检查是否初始化
    void isInit(ResultStruct pResultStruct,String SocketHandle);
    //获取机器人的状态
    void getRobotStatus(RobotStatusStruct pRobotStatusStruct,String SocketHandle,String propertyName);

    //设置机器人属性
    void setRobotProperty(ResultStruct pResultStruct,String SocketHandle,
                          String propertyName,String propertyValue,String dataType);
    //移动机器人到指定点
    void moveToPoint(ResultStruct pResultStruct, String SocketHandle, double dx, double dy);
    //移动机器人到相对位置
    void moveToRelativePoint(ResultStruct pResultStruct, String SocketHandle, double dDistance, double dAngle);
    //机器人控制
    void moveControl(ResultStruct pResultStruct, String SocketHandle, int direction, double distance, double angle);
    //转动底盘到相对地图的x轴的指定角度
    void rotateToAngle(ResultStruct pResultStruct, String SocketHandle, double dAngle);
    //升降杆控制
    void raiseLift(ResultStruct pResultStruct, String SocketHandle, boolean direction);
    void cameraPower(ResultStruct pResultStruct, String SocketHandle, boolean PowerOn);
    //设置任务
    void setTask(ResultStruct pResultStruct, String SocketHandle, CheckPointStruct checkPoints[], int iTaskCount);
    //启动任务
    void startTask(ResultStruct pResultStruct, String SocketHandle, String batchId);
    ///暂停任务
    void pauseTask(ResultStruct pResultStruct, String SocketHandle);
    //停止任务
    void stopTask(ResultStruct pResultStruct, String SocketHandle);
    //启动充电过程
    void startCharge(ResultStruct pResultStruct, String SocketHandle, boolean charge);
    //停止正在进行的移动过程
    void StopMoving(ResultStruct pResultStruct, String SocketHandle);
    //定义响应结构体
